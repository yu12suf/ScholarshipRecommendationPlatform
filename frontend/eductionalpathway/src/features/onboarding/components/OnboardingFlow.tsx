...
import { 
  FileText, Scan, UserCheck, ShieldCheck, 
    ...
    <div className="min-h-screen bg-muted/30 flex flex-col items-center py-12 px-4">
      <div className="w-full max-w-3xl">
        <div className="flex items-center justify-center gap-2 mb-8">
          ...
          <h1 className="text-2xl font-bold text-primary">Setting Up Your Account</h1>
        </div>
        ...
        <div className="flex justify-between mb-12 relative px-4">
          ...
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex flex-col items-center gap-2">
              <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold text-sm transition-all shadow-lg ${
                step >= s ? 'bg-primary text-white scale-110 shadow-primary/20' : 'bg-white text-gray-400 border border-gray-200'
              }`}>
                {step > s ? <CheckCircle2 className="h-6 w-6" /> : s}
              </div>
              <span className={`text-xs font-bold ${step >= s ? 'text-primary' : 'text-gray-400'}`}>
                {s === 1 ? 'Scan Papers' : s === 2 ? 'Verify Who You Are' : 'Finish Setup'}
              </span>
            </div>
          ))}
        </div>
          ...
          {step === 1 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="text-center">
                ...
                <p className="text-muted-foreground mt-2">Upload your CV or school records. Our AI will read the info for you.</p>
              </div>
              ...
              <div className="border-2 border-dashed border-gray-200 rounded-2xl p-10 text-center hover:border-primary/50 transition-colors group relative cursor-pointer">
                ...
                <p className="font-bold text-gray-600">
                  {files.document ? files.document.name : 'Click to upload CV or records'}
                </p>
          ...
          {step === 2 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="text-center">
                ...
                <p className="text-muted-foreground mt-2">Please upload your ID card and a selfie.</p>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:border-accent/50 transition-colors relative cursor-pointer bg-gray-50/50">
                  ...
                  <p className="text-sm font-bold truncate">{files.idCard ? files.idCard.name : 'Upload ID Card'}</p>
                </div>
                <div className="border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:border-accent/50 transition-colors relative cursor-pointer bg-gray-50/50">
                  <input 
                    type="file" 
                  ...
                  <p className="text-sm font-bold truncate">{files.selfie ? files.selfie.name : 'Upload Selfie'}</p>
                </div>
              </div>
          ...
          {step === 3 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 text-center">
              ...
              <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 text-left max-w-md mx-auto">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Identity Confirmed</p>
